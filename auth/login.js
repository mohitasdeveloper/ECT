    ...
    try {
        if (identifier.includes('@')) {
            ...
            if (error) {
                ...
                // Shows exact Supabase Auth error instead of generic fallback
            ...
            // 1. Handle hard network/server crashes safely
            if (error) {
                console.error("Function error:", error);
                // Extract exact error if safe, otherwise fallback gracefully
                const errorMessage = error.message && !error.message.includes("non-2xx") 
                    ? error.message 
            ...
            // 2. Handle specific user errors returned from our Deno function (e.g. "Student ID not found")
            if (data?.error) {
            ...
            // 3. Handle successful session return
            if (data?.session) {
                const { error: sessionError } = await sb.auth.setSession(data.session);

                if (sessionError) {
                    console.error("Session error:", sessionError);
                    // Show exact session error if Supabase rejects setting it
                    showMessage(sessionError.message || "Login failed. Try again.");
                    setLoading(loginButton, false);
                    return;
                }
        ...
        // Extracts real client-side JavaScript crash messages if they occur
