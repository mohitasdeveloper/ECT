...
import { getUserSuggestions, getBlockedUserIds, createNotification } from './data-layer.js';
...
async function submitPost() {
    ...
    try {
        ...
        // 🚀 post_mention notification — mentionedIds already existed (stored on the
        // post row), nothing ever notified the mentioned users.
        mentionedIds.forEach(mentionedId => {
            createNotification({ userId: mentionedId, senderId: currentUser.id, type: 'post_mention', targetId: newPostId });
        });
    ...
    try {
        if (!navigator.onLine) {
            ...
            if (!nextLikedState) {
                ...
                // 🚀 post_like notification — this never existed before. Author id is
                // already sitting in the DOM (post-card.js puts it on .post-options-btn),
                // so no extra query needed.
                const authorId = document.querySelector(`.post-options-btn[data-post-id="${postId}"]`)?.dataset.userId;
                if (authorId) createNotification({ userId: authorId, senderId: currentUser.id, type: 'post_like', targetId: postId });
            }
        }
    } catch (error) {
...
let activeReplyUserId = null;
let currentMentionIds = [];

window.cancelReply = function() {
    activeReplyCommentId = null;
    activeReplyUserId = null;
...
window.prepareReply = function(commentId, userName, userId) {
    activeReplyCommentId = commentId;
    activeReplyUserId = userId || null;
        ...
        <div class="flex items-start gap-3 mb-4 ${paddingLeft}" data-comment-id="${comment.id}" ${parentIdAttr}>
            ...
            <div class="comment-body flex-1 min-w-0 flex flex-col cursor-pointer select-none active:opacity-60 transition-opacity" 
                ...
                <div class="flex items-center gap-4 mt-1">
                    ...
                    <span onclick="event.stopPropagation(); window.prepareReply('${isReply ? comment.parent_comment_id : comment.id}', '${comment.users.full_name}', '${comment.user_id}')" class="text-[11px] font-bold text-on-surface-variant dark:text-gray-500 hover:text-primary transition-colors">Reply</span>
                </div>
            </div>
...
async function submitComment(postId) {
    ...
    try {
        if (!navigator.onLine) {
            ...
            // NORMAL ONLINE SYNC
            const { data: newComment, error } = await supabase.from('post_comments').insert(payload).select('id').single();
            if (error) throw error;

            // 🚀 post_comment / comment_reply / comment_mention notifications — none of
            // these existed before either, despite notifications.js already having full
            // render support for all three.
            if (newComment) {
                if (activeReplyCommentId && activeReplyUserId) {
                    createNotification({ userId: activeReplyUserId, senderId: currentUser.id, type: 'comment_reply', message: content, targetId: postId });
                } else {
                    const authorId = document.querySelector(`.post-options-btn[data-post-id="${postId}"]`)?.dataset.userId;
                    if (authorId) createNotification({ userId: authorId, senderId: currentUser.id, type: 'post_comment', message: content, targetId: postId });
                }
                (currentMentionIds || []).forEach(mentionedId => {
                    createNotification({ userId: mentionedId, senderId: currentUser.id, type: 'comment_mention', message: content, targetId: postId });
                });
            }
...
window.handleCommentLike = async function(commentId, btnElement) {
    ...
    try {
        if (isLiked) {
            ...
            // 🚀 comment_like notification — same story as post likes, never existed.
            const ownerId = btnElement.closest('[data-comment-id]')?.querySelector('[data-comment-owner-id]')?.dataset.commentOwnerId;
            if (ownerId) createNotification({ userId: ownerId, senderId: currentUser.id, type: 'comment_like', targetId: commentId });
        }
    } catch(e) { 
        console.error(e); 
    } finally {
