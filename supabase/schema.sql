...
CREATE TABLE public.messages (
  ...
  hotpost_reply_id uuid, -- set when this message is a reply to a Hotpost (story); NOT a reference to the legacy hotpost_replies table below
  ...
  CONSTRAINT messages_hotpost_reply_id_fkey FOREIGN KEY (hotpost_reply_id) REFERENCES public.hotposts(id) ON DELETE SET NULL,
  CONSTRAINT messages_shared_post_id_fkey FOREIGN KEY (shared_post_id) REFERENCES public.posts(id) ON DELETE SET NULL
);
...
CREATE TABLE public.conversation_settings (
  ...
  CONSTRAINT conversation_settings_unique UNIQUE (user_id, partner_id)
);
