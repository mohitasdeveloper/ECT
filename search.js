...
import { getUserSuggestions } from './data-layer.js';
import { generateSuggestionsHTML } from './feed.js';
...
// 🚀 The Search/Discover tab's empty-query view used to be Featured Services
// only — the container was literally named "Discover Students / Pages" in a
// comment, but nothing ever populated a people section. This app already has a
// complete "suggested for you" system (feed.js's generateSuggestionsHTML +
// getUserSuggestions, used for the feed's own inline widget and the "See All"
// panel) — reused here as-is rather than building a second version of it.
async function fetchDiscoverSuggestions() {
    const container = document.getElementById('discover-suggested-users');
    if (!container || !currentUser) return;
    try {
        const users = await getUserSuggestions(currentUser.id, 12);
        container.innerHTML = generateSuggestionsHTML(users); // returns '' when there's nothing to suggest
    } catch (e) {
        console.error('Error loading Discover suggestions:', e);
    }
}
...
async function fetchFeaturedServices() {
    const container = document.getElementById('discover-featured-services');
    if (!container) return;
