...
async function handleLikeHotpost(event) {
    ...
    try {
        if (wasLiked) {
            ...
            // 🚀 hotpost_like notification — never existed. The story owner's id is
            // already the viewer state's userId, no extra lookup needed.
            createNotification({ userId: currentViewerState.userId, senderId: currentUser.id, type: 'hotpost_like', targetId: post.id });
        }
